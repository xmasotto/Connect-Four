C4.AI = function(_game, _player, _strength) {
	...
	function queryMove(callback) {
		...
		for (var x=0; x<7; x++) {
			for (var y=0; y<7; y++) {
				if (_rack[x][y]) {
					color = _rack[x][y].color == "yellow" ? "x" : "o";
					board[y][x] = color;
				}
			}
		}
	...
	function findAndPlayMove() {
		if (_game.current === _player) {
			queryMove(function(res) {
				console.log(res);
			});

			// Give the previous move's drop animation some time to finish
			setTimeout(function() {
				// console.log(_player.color, token, _game)
				// color, state of board -> return best column
